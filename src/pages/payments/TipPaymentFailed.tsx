...
const TipPaymentFailed: React.FC = () => {
  ...
  // Try to get the creator from stored tip data
  const storedTipData = localStorage.getItem("tipkoro_tip_data");
  const tipData = storedTipData ? JSON.parse(storedTipData) : null;
      ...
      <main className="flex-1 flex items-center justify-center p-6 pt-24">
        <div className="tipkoro-card text-center py-12 max-w-lg w-full">
          ...
          <p className="text-muted-foreground mb-4">
            {tipData?.amount 
              ? `Your tip of ৳${tipData.amount} could not be completed.`
              : "Your tip payment could not be completed."
            }
          </p>
          ...
          <div className="bg-warning/10 border border-warning/30 rounded-xl p-4 mb-8 text-left">
            ...
            <p className="text-sm text-muted-foreground">
              If the amount was deducted from your payment method but the tip 
              didn't go through, please contact us immediately with your 
              transaction details.
            </p>
            <a
              href={`mailto:${SUPPORT_EMAIL}?subject=Tip Payment Issue - Amount Deducted&body=Hi TipKoro Team,%0D%0A%0D%0AMy tip payment was deducted but the transaction didn't complete.%0D%0A%0D%0ATransaction details:%0D%0A- Date/Time: ${new Date().toLocaleString()}%0D%0A- Payment Method: %0D%0A- Amount: ${tipData?.amount || 'Unknown'}%0D%0A%0D%0APlease help resolve this issue.%0D%0A%0D%0AThank you.`}
          ...
          <div className="space-y-3">
            ...
            >
              <Home className="w-4 h-4 mr-2" /> Explore Creators
            </Button>
          </div>
...
export default TipPaymentFailed;
